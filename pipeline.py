...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Local AI Document Processing Pipeline")
    ...
    ingestor = DocumentIngestor(folder_path=args.folder)
    raw_docs = ingestor.process_folder()
    print(f"  → Loaded {len(raw_docs)} document(s): {list(raw_docs.keys())}")
    ...
    results = processor.process_all_documents(raw_docs)

    # Step 3: Save Output.json
    with open(args.output, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
    print(f"\n[STEP 3] Results saved to: {args.output}")
    print(json.dumps(results, indent=2))

    # Step 4: Optional semantic search
    if args.search:
        print(f"\n[STEP 4] Semantic search: \"{args.search}\"")
        engine = LocalSearchEngine(raw_docs)
        hits = engine.search(args.search, top_k=args.top_k)
        for i, hit in enumerate(hits, 1):
            print(f"  {i}. [{hit['score']:.4f}] {hit['filename']}")
